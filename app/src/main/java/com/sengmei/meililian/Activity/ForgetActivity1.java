...
public class ForgetActivity1 extends BaseActivity implements View.OnClickListener {
    ...
    private ForgetActivity1.TimeCount time;
        ...
        switch (v.getId()) {
            case R.id.next:
                ...
                if (StringUtil.isBlank(Phones)) {
                    StringUtil.ToastShow(ForgetActivity1.this, "手机号不能为空");
                } else if (StringUtil.isBlank(Passwords)) {
                    StringUtil.ToastShow(ForgetActivity1.this, "密码不能为空");
                } else if (StringUtil.isBlank(Yanzhengmas)) {
                    StringUtil.ToastShow(ForgetActivity1.this, "请输入验证码");
                } else if (!Passwords.equals(rePasswords)) {
                    StringUtil.ToastShow(ForgetActivity1.this,"密码不一致");
                }else {
            ...
            case R.id.yanzheng:
                ...
                if (StringUtil.checkMobilephone(Phones)){
                    time = new ForgetActivity1.TimeCount(60000, 1000);
                    time.start();
                    yanzheng.setClickable(false);
                    YanZhengShow();
                }else {
                    StringUtil.ToastShow(ForgetActivity1.this, "请填写正确手机号");
                }
    ...
    private void nextShow() {
        ...
        Call<ZhuCeBean> indexdata = mFromServerInterface.Loginforget(Phones, Passwords, rePasswords,Yanzhengmas);
